...
void SX1262::setup() {
  ...
  // Modulation params
  // T1 keeps the proven baseline profile used so far.
  // C1-only gets its own closer-to-spec profile: narrower RX BW, 45 kHz deviation,
  // and shorter preamble. BOTH intentionally keeps the shared baseline profile.
  const bool c1_only = (this->listen_mode_ == LISTEN_MODE_C1);
  const uint32_t bitrate = 100000;
  const uint32_t br = (XTAL_FREQ * 32UL) / bitrate;

  const uint32_t freq_dev = c1_only ? 45000UL : 50000UL;
  const uint32_t fdev = ((uint64_t) freq_dev << 25) / XTAL_FREQ;
  const uint8_t rx_bw = c1_only ? GFSK_RX_BW_234_3 : GFSK_RX_BW_312_0;
  ...
  // Packet params
  const uint16_t preamble_bits = c1_only ? 32 : 64;
