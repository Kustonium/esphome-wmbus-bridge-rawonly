...
void SX1276::setup() {
  ...
  // Keep the proven legacy profile for T1 and BOTH.
  // For C1-only use a distinct RX/AFC bandwidth profile (200 kHz) and 45 kHz deviation.
  // RegRxBw / RegAfcBw encoding comes from the SX1276 FSK bandwidth table:
  //   0x02 = 125.0 kHz, 0x09 = 200.0 kHz.
  const uint8_t rx_bw = c1_only ? 0x09 : 0x02;
  this->spi_write(0x12, {rx_bw, rx_bw});

  const uint16_t freq_dev = c1_only ? 45000 : 50000;
